...
static int8_t active_key_led = -1;
...
void backlight_update_state(void)
{
  ...
  send_color(backlight_state_led & (1<<STATE_LED_SCROLL_LOCK) ? 255 : 0,
             ...
             Device_STATUSLED);
  send_color(backlight_state_led & (1<<STATE_LED_2) ? 255 : 0,
             backlight_state_led & (1<<STATE_LED_1) ? 255 : 0,
             backlight_state_led & (1<<STATE_LED_0) ? 255 : 0,
             Device_STATUSLED);
  send_color(backlight_state_led & (1<<STATE_LED_4) ? 255 : 0,
             backlight_state_led & (1<<STATE_LED_3) ? 255 : 0,
             0,
             Device_STATUSLED);
  sei();
  show();
...
static bool is_alpha_or_space(uint16_t keycode) {
    return ((keycode >= KC_A && keycode <= KC_Z) || keycode == KC_SPC);
}
...
bool process_record_kb(uint16_t keycode, keyrecord_t *record) {
    ...
    if (record->event.pressed) {
        if (is_alpha_or_space(keycode)) {

            // Tắt LED random cũ (nếu còn)
            if (active_key_led >= 0) {
                backlight_state_led &= ~(1 << (STATE_LED_0 + active_key_led));
            }

            // Chọn LED mới 0..4
            active_key_led = rand() % 5;
        ...
        // Nhả phím → tắt LED random nếu nó đang bật
        if (active_key_led >= 0) {
            backlight_state_led &= ~(1 << (STATE_LED_0 + active_key_led));
            active_key_led = -1;
            backlight_update_state();
